	...
	if (is_tx_onyx) {
		...
		if (tx.onyx_type == parameters::ONYX_TYPE_TRANSFER) {
			...
			if (!zk::Halo2ProofSystem::verify_and_extract_transfer(tx.onyx_envelope,
			        parameters::ONYX_MERKLE_DEPTH, parameters::ONYX_TRANSFER_CIRCUIT_K, &verified))
		...
		if (tx.onyx_type == parameters::ONYX_TYPE_PROGRAM_DEPLOYMENT) {
			...
			if (!zk::Halo2ProofSystem::verify_program_deployment(tx.onyx_envelope,
			        parameters::ONYX_MERKLE_DEPTH, parameters::ONYX_PROGRAM_CIRCUIT_K, &verified))
		...
		if (tx.onyx_type == parameters::ONYX_TYPE_STANDARD_PROGRAM_CALL) {
			// Registry activation, the composite proof, and its prior->next state transition are
			// checked atomically against the current snapshot below. Standard calls have zero fee.
			return 0;
		}
...
bool BlockChainState::add_transaction(const Hash &tid, const Transaction &tx, const BinaryArray &binary_tx,
	...
	if (tx.version == m_currency.onyx_transaction_version && tx.onyx_type == parameters::ONYX_TYPE_TRANSFER) {
		if (!zk::Halo2ProofSystem::verify_and_extract_transfer(tx.onyx_envelope,
		        parameters::ONYX_MERKLE_DEPTH, parameters::ONYX_TRANSFER_CIRCUIT_K, &onyx_delta))
			throw ConsensusError("Invalid Onyx authorized transfer");
		...
		if (!zk::Halo2ProofSystem::verify_apply_transfer(snapshot, parameters::ONYX_ANCHOR_WINDOW_BLOCKS,
		        tx.onyx_envelope, parameters::ONYX_MERKLE_DEPTH, parameters::ONYX_TRANSFER_CIRCUIT_K, network,
	...
	if (tx.version == m_currency.onyx_transaction_version &&
		...
		if (!zk::Halo2ProofSystem::verify_apply_program_deployment(snapshot,
		        ...
		        parameters::ONYX_PROGRAM_CIRCUIT_K, network, next_block_height, &dry_run_snapshot, &dry_run_fee,
		        &dry_run_program) ||
	...
	for (const auto &nullifier : onyx_delta.nullifiers)
		if (!m_memory_state_onyx_nf_tx.insert(std::make_pair(nullifier, tid)).second)
			all_inserted = false;
	for (const auto &state_key : onyx_standard_state_keys)
		if (!m_memory_state_onyx_standard_state_tx.insert(std::make_pair(state_key, tid)).second)
			all_inserted = false;
...
void BlockChainState::remove_from_pool(Hash tid) {
	...
	if (tx.version == m_currency.onyx_transaction_version && tx.onyx_type == parameters::ONYX_TYPE_TRANSFER) {
		...
		invariant(zk::Halo2ProofSystem::verify_and_extract_transfer(tx.onyx_envelope,
		              parameters::ONYX_MERKLE_DEPTH, parameters::ONYX_TRANSFER_CIRCUIT_K, &delta),
	...
	if (tx.version == m_currency.onyx_transaction_version &&
		...
		invariant(zk::Halo2ProofSystem::extract_authenticated_standard_program_delta(
		    ...
		    "stored Onyx standard program call failed authenticated extraction");
		for (const auto &nullifier : delta.nullifiers)
			if (m_memory_state_onyx_nf_tx.erase(nullifier) != 1)
				all_erased = false;
		for (const auto &state_key : state_keys)
			if (m_memory_state_onyx_standard_state_tx.erase(state_key) != 1)
				all_erased = false;
	}
	if (tx.version == m_currency.onyx_transaction_version &&
	    tx.onyx_type == parameters::ONYX_TYPE_PROGRAM_DEPLOYMENT) {
...
void BlockChainState::redo_transaction(uint8_t major_block_version, bool coinbase, const Transaction &transaction,
	...
	if (transaction.version == m_currency.onyx_transaction_version) {
		...
		if (transaction.onyx_type == parameters::ONYX_TYPE_TRANSFER) {
			uint64_t fee = 0;
			if (!zk::Halo2ProofSystem::verify_apply_transfer(snapshot, parameters::ONYX_ANCHOR_WINDOW_BLOCKS,
			        transaction.onyx_envelope, parameters::ONYX_MERKLE_DEPTH,
			        parameters::ONYX_TRANSFER_CIRCUIT_K,
			...
			if (!zk::Halo2ProofSystem::verify_apply_program_deployment(snapshot,
			        ...
			        parameters::ONYX_MERKLE_DEPTH, parameters::ONYX_PROGRAM_CIRCUIT_K, network,
		...
		} else if (transaction.onyx_type == parameters::ONYX_TYPE_STANDARD_PROGRAM_CALL) {
			zk::Halo2ProofSystem::VerifiedTransferDelta contextual_delta;
			if (!zk::Halo2ProofSystem::verify_apply_standard_program_transaction(snapshot,
			        transaction.onyx_envelope, parameters::ONYX_MERKLE_DEPTH,
			        parameters::ONYX_PROGRAM_CIRCUIT_K,
			        network, delta_state->get_block_height(), &next_snapshot, &contextual_delta))
				throw ConsensusError("Onyx standard program state transition rejected");
		...
		if (tit->version == m_currency.onyx_transaction_version) {
			...
			if (tit->onyx_type == parameters::ONYX_TYPE_TRANSFER ||
				...
				if (tit->onyx_type == parameters::ONYX_TYPE_TRANSFER) {
					...
					std::vector<std::array<uint8_t, 32>> state_keys;
					invariant(zk::Halo2ProofSystem::extract_authenticated_standard_program_delta(
					              tit->onyx_envelope, &onyx_delta, &state_keys),
					    "accepted Onyx standard program call failed authenticated extraction");
					for (const auto &state_key : state_keys) {
						auto state_conflict = m_memory_state_onyx_standard_state_tx.find(state_key);
						if (state_conflict != m_memory_state_onyx_standard_state_tx.end())
							conflicting_pool_transactions.insert(state_conflict->second);
					}
				}
				for (const auto &nullifier : onyx_delta.nullifiers) {
