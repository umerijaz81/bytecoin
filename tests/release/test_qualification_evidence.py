...
class QualificationEvidenceTest(unittest.TestCase):
    ...
    def test_typed_attestation_rejects_nested_non_finite_number(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = pathlib.Path(temporary)
            document = self.audit(root, "A Labs", "a.txt")
            document["measurement"] = {"failure_rate": float("nan")}
            ...
            errors = qualification_evidence.verify_gate(
                ...
                ["audit-a.json", second],
                root,
                tracked_paths={"audit-a.json", second, "a.txt", "b.txt"},
            )
            self.assertTrue(any("non-finite JSON number" in error for error in errors), errors)
            self.assertTrue(
                any("passed status requires 2 typed JSON attestation" in error for error in errors),
                errors,
            )

    def test_typed_attestation_rejects_exponent_overflow(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = pathlib.Path(temporary)
            document = self.audit(root, "A Labs", "a.txt")
            raw = json.dumps(document)
            raw = raw[:-1] + ', "measurement": {"failure_rate": 1e999}}'
            (root / "audit-a.json").write_text(raw, encoding="utf-8")
            second = self.write_document(root, "audit-b.json", self.audit(root, "B Labs", "b.txt"))
            errors = qualification_evidence.verify_gate(
                "independent-audits",
            ...
            self.assertTrue(any("exceeds finite range" in error for error in errors), errors)
            self.assertTrue(
                any("passed status requires 2 typed JSON attestation" in error for error in errors),
                errors,
            )

    def test_typed_attestation_rejects_excessive_nesting(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = pathlib.Path(temporary)
            document = self.audit(root, "A Labs", "a.txt")
            raw = json.dumps(document)
            nested = ("[" * 65) + "0" + ("]" * 65)
            raw = raw[:-1] + f', "measurement": {nested}}}'
            (root / "audit-a.json").write_text(raw, encoding="utf-8")
            second = self.write_document(root, "audit-b.json", self.audit(root, "B Labs", "b.txt"))
            errors = qualification_evidence.verify_gate(
                "independent-audits",
            ...
            self.assertTrue(any("nesting limit" in error for error in errors), errors)
            self.assertTrue(
                any("passed status requires 2 typed JSON attestation" in error for error in errors),
                errors,
            )
