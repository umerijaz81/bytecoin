...
| Authenticated browser-wallet storage | `src/Core/WalletHD.*`, `WalletNodeExt.cpp`, `src/platform/IndexDB.*` | Emscripten wallet seeds are stored in a password-derived ChaCha20 envelope with keyed authentication, strict field/size checks and mandatory non-empty passwords. Plaintext records migrate before open succeeds, and password changes report storage failures instead of false success. |
