...
| Privacy-minimized wallet sync | `src/Core/WalletSync.cpp`, wallet/node RPC docs | `--wallet-sync-privacy` bootstraps with the canonical zero/genesis placeholder and then uses height-addressed static pages, avoiding wallet creation timestamps and sparse-chain fingerprints. It fails closed if valid static pages are unavailable and omits wallet-local mempool hashes by reconciling the node's full response locally. |
...
`PRIVACY_SYNC_UNAVAILABLE` rather than silently weakening the selected privacy mode. Mempool sync
also sends an empty known-hash set and locally derives removals from the node's complete response,
preventing exact transaction-awareness fingerprinting.
