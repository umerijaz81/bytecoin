...
|---|---|---|
| O0 proof foundation | Exact-pinned vendored Halo2/Pasta crate, active fail-closed proof-system seam, bounded panic-contained C ABI, CMake integration, locked offline three-platform workflow, primitive vectors, malformed-input/batch-shape tests, and an automated ASan/UBSan/libFuzzer gate with deterministic C++ parser/Onyx envelope seeds. Structured consensus verification and extraction entry points clear fixed caller outputs and element counts before every post-validation failure. | External dependency/proof-boundary review, sustained independently reviewed fuzz campaigns over valid proof envelopes and reproducible release artifacts. |
