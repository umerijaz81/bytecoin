...
| O3 wallet | Seed/key hierarchy, Onyx addresses, full viewing keys, scanning, historical witnesses, proving, recovery snapshots, pending reservations, native/token balances and wallet RPC. Encrypted native backup/password rotation and view-only recovery preserve keys, address count, labels and queued payments under automated qualification. | End-to-end multi-node devnet recovery plus hardware-wallet and operator acceptance tests. |
